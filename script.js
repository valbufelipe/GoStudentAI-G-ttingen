...
if (titulo === "CV Helper") {
  ...
  let eventsTool = document.getElementById("events-tool");
...
placeholderTool.style.display = "none";
  ...
  if (titulo === "Events Radar") {
  eventsTool.style.display = "block";

  document.getElementById("resultado").innerText =
    "Fülle das Events-Formular aus und generiere Ideen für Göttingen.";
...
function generarEventText() {
  let day = document.getElementById("event-day").value.trim();
  let type = document.getElementById("event-type").value.trim();
  let budget = document.getElementById("event-budget").value.trim();
  let vibe = document.getElementById("event-vibe").value.trim();
  let language = document.getElementById("event-language").value;

  if (!day || !type) {
    document.getElementById("resultado").innerText =
      "Bitte fülle Tag und Art der Aktivität aus. / Please fill in day and activity type.";
...
Events Radar · Göttingen

Für ${day} und den Bereich "${type}" würde ich nach Aktivitäten suchen, die ${vibe || "angenehm und passend"} sind.

Budget: ${budget || "flexibel"}

Ideen:
1. Schau nach Events beim Göttingen Campus, Uni-Gruppen oder studentischen Initiativen.
2. Prüfe Bars, Cafés oder Kulturorte in der Innenstadt.
3. Suche nach kostenlosen oder günstigen Veranstaltungen.
4. Frag in WhatsApp-/Telegram-Gruppen von Studierenden.
5. Plane eine einfache Alternative: Café, Spaziergang, Spieleabend oder Uni-Event.

Mini-Checklist:
- Ort prüfen
- Uhrzeit prüfen
- Eintritt/Kosten prüfen
- Mit Freunden teilen
- Früh genug losgehen`;
...
For ${day} and the category "${type}", I would look for activities that feel ${vibe || "pleasant and suitable"}.

Budget: ${budget || "flexible"}

Ideas:
1. Check events from Göttingen Campus, university groups, or student initiatives.
2. Look at bars, cafés, or cultural places in the city center.
3. Search for free or low-cost events.
4. Ask in student WhatsApp or Telegram groups.
5. Plan a simple alternative: café, walk, game night, or university event.
